...
#we can see we have 28% A, 19% B, 17% C, 16% D and 18% E
...
lapply(training[ , 8:59], function(x) summary(aov(x ~ classe, data = training)))

anovas <- data.frame(Predictors <- as.character(), 
                     F_value <- as.numeric(), 
                     p_value <- as.numeric())
aov.temp <- aov(training[ , 8] ~ classe, data = training)
x <- c(names(training[i]),
       summary(aov.temp)[[1]][["F value"]][[1]], 
       summary(aov.temp)[[1]][["Pr(>F)"]][[1]])
anovas <- rbind(anovas, x)

for (i in 9:59) {
    aov.temp <- aov(training[ , i] ~ classe, data = training)
    x <- c(names(training[i]),
           summary(aov.temp)[[1]][["F value"]][[1]], 
           summary(aov.temp)[[1]][["Pr(>F)"]][[1]])
    levels(anovas$X.roll_belt.) <- c(levels(anovas$X.roll_belt.), names(training[i]))
    anovas <- rbind(anovas, x)
    x <- NULL
} 
anovas
...
lapply(training[ , 8:59], spec.cor(training))
...
anovas <- as.data.frame(sapply(training[ , 8:59], function(x)
    summary(aov(x ~ classe, data = training))[[1]][["F value"]][[1]]))
...
d.plots <- function(predictor, label) {
    qplot(predictor, colour = classe, data = training, geom = "density") + 
        xlab(label) +
        ylab("Density") +
        ggtitle(label) +
        theme_bw()
...
g1 <- d.plots(training$pitch_forearm, "Forearm Pitch")
g2 <- d.plots(training$magnet_belt_y, "Belt Magnetometer, Y-axis")
g3 <- d.plots(training$magnet_arm_x, "Arm Magnetometer, X-axis")
g4 <- d.plots(training$magnet_arm_y, "Arm Magnetometer, Y-axis")
g5 <- d.plots(training$accel_arm_x, "Arm Acceleration, X-axis")
g6 <- d.plots(training$accel_forearm_x, "Forearm Acceleration, X-axis")

g7 <- d.plots(training$yaw_belt, "Belt Yaw")
g8 <- d.plots(training$roll_belt, "Belt Roll")
g9 <- d.plots(training$magnet_dumbbell_z, "Dumbbell Magnetometer, Z-axis")
...
set.seed(125)
modFit <- train(classe ~ ., method = "rpart", data = training[ , c(2, 8:60)])
...
set.seed(567)
fit <- randomForest(as.factor(classe) ~ ., data = training[ , c(2, 8:60)], importance=TRUE, ntree=2000)
...
user.plots <- function(predictor, label) {
    qplot(predictor, colour = user_name, data = training, geom = "density") + 
        xlab(label) +
        ylab("Density") +
        ggtitle(label) +
        theme_bw()
...
user.plots(training$pitch_forearm, "Forearm Pitch")

qplot(pitch_forearm, colour = user_name, 
      data = training1[training$user_name != "adelmo", ], 
      geom = "density") + 
    xlab("Forearm Pitch") +
    ylab("Density") +
...
# train the model 
model <- train(classe ~ ., data=training[, 8:60], trControl=train_control, method="nb")
model <- train(classe ~ ., data=training[, 8:60], method="nb")
# make predictions
predictions <- predict(model, training[,8:59])
# summarize results
confusionMatrix(predictions, training$classe)$byClass
...
folds <- createFolds(training$classe, k = 10, list = TRUE, returnTrain = FALSE)
model <- train(classe ~ ., data=training[, 8:60], method="nb")
